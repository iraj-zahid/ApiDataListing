...
const Cart = () => {
    ...
    const filterCart = loginUserData && cartItems && cartItems.filter((items) => items.email === loginUserData.email)
    ...
    // const filterCart = cartItems.filter((items) => items.email === loginUserData.email)
    return(
        <>
        <div className="w-full min-h-screen p-[2%] flex flex-col bg-[#F2F3F2] dark:bg-[#101014] dark:text-white">
             <div className="w-full flex items-center justify-end ">
             <button  onClick={removeAllFromCart} className="rounded-2xl sm:p-[1%] p-[2%] bg-indigo-600 hover:bg-indigo-700 w-[15%] min-[750px]:w-[10%] text-white font-bold max-[500px]:font-semibold mt-2  ml-3 rounded text-lg flex items-center justify-center">
             Clear
             </button>
             </div>
             ...
             {filterCart && filterCart.map((cartProduct) => {
               return (
                <>
                <div className="IBM w-full gap-[3%] max-[500px]:gap-[5%] rounded-lg mt-[1%] mb-[1%] flex justify-center items-center border-b-[1px] border-black">
                    ...
                    <div className="w-[10%] max-[500px]:w-[8%] max-[500px]:ml-[1%] min-[500px]:ml-[3%] flex items-center justify-center p-[1%] text-black rounded-lg"><p className="rounded-lg p-[6%] bg-[#a1a1a162] cursor-pointer"><IoMdAdd className="dark:text-white max-[500px]:text-lg" onClick={() => addQuant(cartProduct)}/></p><p className="IBM rounded-lg p-[5%] dark:text-white max-[500px]:text-lg">{cartProduct.quant}</p> <p className="p-[6%] rounded-lg bg-[#a1a1a162] cursor-pointer"><FaMinus className="dark:text-white max-[500px]:text-lg" onClick={() => decQuant(cartProduct)}/></p></div>
                ...
                <div className="w-full p-[1%] flex items-center justify-end mt-[3%]">
                    <div className="w-[40%] max-[750px]:w-[60%] p-[1%] flex items-center justify-center flex-col ">
                        ...
                        <button className="w-[70%] text-2xl text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-4 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">CheckOut</button>
                    </div>
                </div>
        </div>
        </>
